...
# Especifica que se trata de una cadena de caracteres y se pasa al entry del nombre en este caso
minombre = StringVar()
...
# Cuadros de texto
# Con textvariable consigo que el cuadro este asociado al valor de la variable
textoNombre = Entry(raiz, textvariable=minombre)
...
barradescroll = Scrollbar(
    raiz, command=comentarioText.yview)  # Crear la barra de scroll
# posicion y hacer que se mueva la barra con los botones
barradescroll.grid(row=4, column=2, sticky="nsew")
# Hace que se posicione la barra dependiendo donde se encuentre el cursor
comentarioText.config(yscrollcommand=barradescroll.set)
